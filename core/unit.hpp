...
#ifdef _SCC_DEBUG
	if(unit_type >= UNIT_TYPE_COUNT) {
		toErrorLog("DEBUG: (UNIT::addAvailible): Value unit_type out of range.");return;
	}
	if(availible[unit_type]+unit_availible>MAX_TOTAL_UNITS) {
		toErrorLog("DEBUG: (UNIT::addAvailible): Value unit_availible out of range.");return;
	}
...
#ifdef _SCC_DEBUG
	if(unit_type >= UNIT_TYPE_COUNT) {
		toErrorLog("DEBUG: (UNIT::addTotal): Value unit_type out of range.");return;
	}
	if(total[unit_type]+unit_total > MAX_TOTAL_UNITS) {
		toErrorLog("DEBUG: (UNIT::addTotal): Value unit_total out of range.");return;
	}
#endif
	total[unit_type] += unit_total;
}
