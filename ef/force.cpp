...
ForceWindow::ForceWindow(UI_Object* force_parent, const unsigned int game_number, const unsigned int game_max, const unsigned int player_number, const unsigned int player_max) :
	...
	currentGoalUnit(0),
	startLine(0),
	menuRadio(new UI_Radio(this, Rect(getRelativeClientRectPosition() + Size(5,5), Size()), Size(), HORIZONTAL_GROUP)),
	nongoalsText(new UI_StaticText(this, NON_GOALS_STRING, Rect(), Size(), FORCE_TEXT_COLOR, SMALL_BOLD_FONT, DO_NOT_ADJUST)),
	goalsText(new UI_StaticText(this, GOALS_STRING, Rect(), Size(), FORCE_TEXT_COLOR, SMALL_BOLD_FONT, DO_NOT_ADJUST)),
	legend(new UI_StaticText(this, TIME_LEGEND_STRING, Rect(Point(200, 0), Size()), Size(), FORCE_TEXT_COLOR, SMALL_FONT, DO_NOT_ADJUST)), // TODO
	goalName(new UI_Button(this, Rect(), Size(), TEXT_BUTTON, NULL_BITMAP, STATIC_BUTTON_MODE, CHOOSE_GOAL_NAME_STRING, DO_NOT_ADJUST, SMALL_SHADOW_BOLD_FONT, AUTO_SIZE)),
	goalForceList(),
	nongoalForceList(),
	markedUnit(0),
	oldMarkedUnit(0),
...
//	locationMenu(new LocationMenu(this, anarace, getRelativeClientRect())),
	...
	assignGoal(-1)
{	
	menuButton[UNIT_MENU] = new UI_Button(menuRadio, Rect(Point(), Size(21,20)), Size(), MENU_BUTTON, ADD_BITMAP, STATIC_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST);
	menuButton[GOAL_MENU] = new UI_Button(menuRadio, Rect(Point(), Size(21,20)), Size(), MENU_BUTTON, LOAD_BITMAP, STATIC_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST);
	menuButton[SAVE_GOALS] = new UI_Button(menuRadio, Rect(Point(), Size(21,20)), Size(), MENU_BUTTON, SAVE_BITMAP, STATIC_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST),
...
void ForceWindow::process()
{
	...
	if(ForceEntry::changed)
	{
			...
			if((ForceEntry::changed == NUMBER_HAS_CHANGED) || ((ForceEntry::changed==GOAL_TIME_HAS_CHANGED)&&(ForceEntry::forceEntryCount<0)))
			{
				...
				else 
				{
					...
					if(error_list.size()>0)
					{
						...
						{
							UI_Object::sound.playSound(ERROR_SOUND, (getAbsolutePosition() + getSize()/2).x);
					...
					else
					{
						if(ForceEntry::forceEntryCount >0)
							UI_Object::msgList.push_back(UI_Object::theme.lookUpFormattedString(ADDED_ONE_GOAL_STRING, GAME::lookUpUnitString(anarace->getRace(), ForceEntry::forceEntryUnit)));
						else if(ForceEntry::forceEntryCount <0)
							UI_Object::msgList.push_back(UI_Object::theme.lookUpFormattedString(REMOVED_ONE_GOAL_STRING, GAME::lookUpUnitString(anarace->getRace(), ForceEntry::forceEntryUnit)));
						setChangedFlag();
					}
		...
		} else if((!ForceEntry::forceEntryIsGoal)&&(ForceEntry::changed == NUMBER_HAS_CHANGED))
		{
			...
			UI_Object::msgList.push_back(UI_Object::theme.lookUpFormattedString(MOVED_NON_GOAL_STRING, GAME::lookUpUnitString(anarace->getRace(), ForceEntry::forceEntryUnit)));
	...
	if(ForceEntry::doCompleteSound)
	{
		...
		UI_Object::sound.playSound(COMPLETE_SOUND, (getAbsolutePosition() + getSize()/2).x);
	}
...
// ------ CHECK FOR PRESSED UNIT ITEM
	...
	if((pressed = unitMenu->getPressedItem())>=0)
	{
		...
		{
			UI_Object::msgList.push_back(UI_Object::theme.lookUpFormattedString(ADDED_GOAL_STRING, GAME::lookUpUnitString(anarace->getRace(), pressed)));
			setChangedFlag();
		}
	}
...
void ForceWindow::needSaveBox()
{
	...
	if(savingGoal)
		...
		UI_Window::saveBoxParameter.saveboxText = NULL_STRING;
...
//			if ((!anarace->getGoal()->isGoal(i)) && ((anarace->getGoal()->getIsBuildable(i))|| ( anarace->getLocationTotal(0,i))))
			if ((anarace->getGoal()->getIsDisplayed(i))&&(anarace->getLocationTotal(loc, i))&&(!checkedUnits[i]))
			{
				...
				if((*forceEntry)->isLeftClicked())
				{
					...
					addTime = 0;
					addLocation = 0;
					msgWindow->addMessage(UI_Object::theme.lookUpFormattedString(MOVED_NON_GOAL_STRING, GAME::lookUpUnitString(anarace->getRace(), i)));
				}
