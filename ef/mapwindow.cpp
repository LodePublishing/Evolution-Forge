...
MapWindow::MapWindow(UI_Object* mapwindow_parent):
	UI_Window(mapwindow_parent, MAP_WINDOW_TITLE_STRING, theme.lookUpGlobalRect(MAP_WINDOW), theme.lookUpGlobalMaxHeight(MAP_WINDOW), NOT_SCROLLED),
	map(new BASIC_MAP()),
...
void MapWindow::reloadOriginalSize()
{
	setOriginalRect(theme.lookUpGlobalRect(MAP_WINDOW));
	setMaxHeight(theme.lookUpGlobalMaxHeight(MAP_WINDOW));
...
void MapWindow::calculateMap()
{
	map->setName(mapName->getText()->getString());
	map->setMaxLocations(maxLocations->getNumber());
	map->setMaxPlayer(maxPlayer->getNumber());
	
	for(unsigned int i = 0; i < map->getMaxLocations(); i++)
	{
		map->setLocationName(i, locationName[i]->getText()->getString());
//		map->setLocationMineralPatches(i, ...);
//		map->setLocationVespeneGeysirs(i, ...);
//		map->setLocationMineralDistance(i, ...);
//		for(unsigned int j = 0; j < map->maxLocations; j++)
//			map->setLocationDistance(i, j, ...);
		map->calculateLocationsDistances();
	}
}

void MapWindow::resetData()
{}

void MapWindow::closeMenus()
{
...
void MapWindow::loadFailsafeDefaults()
{
...
#endif 
void MapWindow::draw(DC* dc) const
{
	if(!isShown())
		return;
	UI_Window::draw(dc);
...
#if 0
void MapWindow::forceLanguageChange()
{
	languageHasChanged=true;
}
#endif
