...
//		for(unsigned int j = MAX_LANGUAGES;j--;)
	...
	for(unsigned int i = MAX_ANIMATIONS;i--;)
...
//	SDL_FreeCursor(cursorList[CLOCK_CURSOR][1]);
	...
	toInitLog("* " + lookUpString(END_FREEING_ANIMATIONS_STRING));
...
void UI_Theme::unloadGraphics()
{
	for(std::list<BitmapEntry>::iterator i = loadedBitmaps.begin(); i!=loadedBitmaps.end(); ++i)
		if(!i->used)
		{
			...
			i->used = false;
	for(std::list<AnimationEntry>::iterator i = loadedAnimations.begin(); i!=loadedAnimations.end(); ++i)
		if(!i->used)
		{
			for(unsigned int j = MAX_ANIMATIONS;j--;)
				if(i->animation == animationList[j])
					animationList[j] = NULL;
		} else if(i->animation!=NULL)
			i->used = false;
}
...
const eDataType get_data_type(const std::string& item)
{
	...
	if(item=="@ANIMATIONS") return(ANIMATION_DATA_TYPE);else
...
void UI_Theme::loadData(const std::string& data_file, const std::string& bitmap_dir, const std::string& animation_dir, const std::string& font_dir, DC* dc)
{
	const unsigned int MAX_PARAMETERS = 50;
	char* buffer;
	std::string parameter[MAX_PARAMETERS];
...
// sub-sub-items already closed -> close sub-item
			...
			else if(sub_mode == ZERO_SUB_DATA_TYPE)
			{
				...
				} else if(mode == BITMAP_DATA_TYPE)
				{
					...
					mode = ZERO_DATA_TYPE;
				} else if(mode == ANIMATION_DATA_TYPE)
				{
					if(!loading_animations)
					{
						loading_animations = true;
						toInitLog("  - " + lookUpString(START_LOAD_ANIMATIONS_STRING));
					}					
					std::map<std::string, std::list<std::string> > block;
					pFile.seekg(old_pos);
					if(!parse_block_map(pFile, block))
					{
						toErrorLog("WARNING (UI_Theme::loadDataFile()): No concluding @END for @THEME_ANIMATION_DATA_TYPE block was found in file " + data_file + " => trying to parse what we have so far.");
					}
					for(unsigned int j = 0; j < MAX_ANIMATIONS; j++)
					{
						std::map<std::string, std::list<std::string> >::iterator i;
						if((i = block.find(animationIdentifier[j])) != block.end())
						{
							i->second.pop_front(); // Identifier loeschen
							if(!i->second.size())
							{
								toErrorLog("WARNING (UI_Theme::loadDataFile()): String identifier without string for @THEME_ANIMATION_DATA_TYPE block was found in file " + data_file + " => ignoring line.");
							} else
							{
								std::string name = i->second.front();
								i->second.pop_front();
								if((name.size()<4)||(name[name.size()-4]!='.'))
									name = animation_dir + name + ".mpg";
								else name = animation_dir + name;
/*									bool solid = false;
								bool scale = false;
								if(i->second.size())
								{
...
//								entry.solid = solid;
//								entry.scale = scale;
								loadedAnimations.push_back(entry);
								animationAccessTable[j] = &(loadedAnimations.back());
							}
			                                block.erase(i);
		        	                }
					}
...
// reload
	{
		toInitLog("Loading " + bitmapAccessTable[currentResolution][currentColorTheme][id]->name);
...
SMPEG_Info& UI_Theme::lookUpAnimationInfo(const eAnimation id)
{
	// TODO Fehler...
	lookUpAnimation(id);
	return(animationAccessTable[id]->info);
	
}

SMPEG* UI_Theme::lookUpAnimation(const eAnimation id)
{
#ifdef _SCC_DEBUG
	if((id<0)||(id>=MAX_ANIMATIONS)) {
		toErrorLog("ERROR: (UI_Theme::lookUpAnimation) id out of range.");return(NULL); // TODO
	}
#endif
...
// reload
	{
		toInitLog("Loading " + animationAccessTable[id]->name);
		
		SMPEG* temp = NULL;
		if(animationAccessTable[id]==NULL)
		{
			toErrorLog("ERROR (UI_Theme::lookUpAnimation()): Animation " + animationIdentifier[id] + " was not initialized. Check 'settings/ui/default.ui' and 'data/animations'.");
			return(NULL);
		} else 
		// picture already loaded?
		if((animationAccessTable[id]->animation != NULL) && (animationAccessTable[id]->animation))
		{
			temp = animationAccessTable[id]->animation;
		} else
		// no? then load it.
		// ....
		
		temp = SMPEG_new(animationAccessTable[id]->name.c_str(), &animationAccessTable[id]->info, 0);
		if ( SMPEG_error(temp) )
	        {
			toErrorLog("ERROR (UI_Theme::lookUpAnimation()): Could not load Animation " + animationAccessTable[id]->name + " : " + SMPEG_error(temp));
			SMPEG_delete(temp);
			return(NULL);
		}
		if(temp == NULL)
			toErrorLog("error"); // TODO

		SMPEG_enableaudio(temp, 0);
		SMPEG_enablevideo(temp, 1);
		SMPEG_setvolume(temp, 100);

/*		if(!animationAccessTable[id]->solid)
		{
			SDL_SetColorKey(temp, SDL_SRCCOLORKEY , SDL_MapRGB(temp->format, 0, 0, 0));
//			SDL_SetAlpha(temp, SDL_SRCALPHA, 128);
		}
		if((bitmapAccessTable[id]->scale) && (currentResolution != (eResolution)(MAX_RESOLUTIONS-1)))
		{
			Size size_orig = getResolutionSize(currentResolution);
			Size size_max = getResolutionSize((eResolution)(MAX_RESOLUTIONS-1));
			double zoomx = (double)(size_orig.getWidth()) / (double)(size_max.getWidth());
		...
		{
			animationAccessTable[id]->animation = temp;
			animationList[id] = temp;
		}
	}
	animationAccessTable[id]->used = true;
	SMPEG_getinfo(animationList[id], &animationAccessTable[id]->info);
...
const eAnimation UI_Theme::getAnimationFromIdentifier(const std::string& identifier) const
{
	for(unsigned int j = MAX_ANIMATIONS; j--;)
		if(animationIdentifier[j] == identifier)
			return((eAnimation)j);
...
//	bitmapIdentifier[OPEN_TREE_BITMAP] = "OPEN_TREE_BITMAP";
	...
	bitmapIdentifier[INTRO_MAIN_BITMAP] = "INTRO_MAIN_BITMAP";
	bitmapIdentifier[INTRO_SDL_BITMAP] = "INTRO_SDL_BITMAP";
	bitmapIdentifier[INTRO_SDL_TTF_BITMAP] = "INTRO_SDL_TTF_BITMAP";
	bitmapIdentifier[INTRO_SDL_IMAGE_BITMAP] = "INTRO_SDL_IMAGE_BITMAP";
	bitmapIdentifier[INTRO_GUI_BITMAP] = "INTRO_GUI_BITMAP";
	bitmapIdentifier[INTRO_SMPEG_BITMAP] = "INTRO_SMPEG_BITMAP";
#ifdef _FMOD_SOUND
	bitmapIdentifier[INTRO_FMOD_BITMAP] = "INTRO_FMOD_BITMAP";
#elif _SDL_MIXER_SOUND
	bitmapIdentifier[INTRO_SDL_MIXER_BITMAP] = "INTRO_SDL_MIXER_BITMAP";
#endif
}

void UI_Theme::initAnimationIdentifier()
{
	for(unsigned int i = MAX_ANIMATIONS; i--;)
		animationIdentifier[i] = "null";

	animationIdentifier[NULL_ANIMATION] = "NULL_ANIMATION";
	animationIdentifier[INTRO_MAIN_ANIMATION] = "INTRO_MAIN_ANIMATION";
	animationIdentifier[INTRO_SDL_ANIMATION] = "INTRO_SDL_ANIMATION";
	animationIdentifier[INTRO_SDL_TTF_ANIMATION] = "INTRO_SDL_TTF_ANIMATION";
	animationIdentifier[INTRO_SDL_IMAGE_ANIMATION] = "INTRO_SDL_IMAGE_ANIMATION";
	animationIdentifier[INTRO_GUI_ANIMATION] = "INTRO_GUI_ANIMATION";
	animationIdentifier[INTRO_SMPEG_ANIMATION] = "INTRO_SMPEG_ANIMATION";
#ifdef _FMOD_SOUND
	animationIdentifier[INTRO_FMOD_ANIMATION] = "INTRO_FMOD_ANIMATION";
#elif _SDL_MIXER_SOUND
	animationIdentifier[INTRO_MIXER_ANIMATION] = "INTRO_MIXER_ANIMATION";
#endif
}
...
void UI_Theme::initStringIdentifier()
{
	...
	stringIdentifier[START_LOADING_STRING] = "START_LOADING_STRING";
        ...
        stringIdentifier[START_LOAD_ANIMATIONS_STRING] = "START_LOAD_ANIMATIONS_STRING";
	...
	stringIdentifier[END_FREEING_ANIMATIONS_STRING] = "END_FREEING_ANIMATIONS_STRING";
	...
	stringIdentifier[INTRO_MAIN_STRING] = "INTRO_MAIN_STRING";
	stringIdentifier[INTRO_SDL_STRING] = "INTRO_SDL_STRING";
	stringIdentifier[INTRO_SDL_TTF_STRING] = "INTRO_SDL_TTF_STRING";
	stringIdentifier[INTRO_SDL_IMAGE_STRING] = "INTRO_SDL_IMAGE_STRING";
	stringIdentifier[INTRO_GUI_STRING] = "INTRO_GUI_STRING";
	stringIdentifier[INTRO_SMPEG_STRING] = "INTRO_SMPEG_STRING";
//#ifdef _FMOD_SOUND
	stringIdentifier[INTRO_FMOD_STRING] = "INTRO_FMOD_STRING";
//#elif _SDL_MIXER_SOUND
	stringIdentifier[INTRO_SDL_MIXER_STRING] = "INTRO_SDL_MIXER_STRING";
//#endif
	stringIdentifier[INTRO_CLICK_STRING] = "INTRO_CLICK_STRING";
