...
StatisticsWindow::StatisticsWindow(UI_Object* parent, ANARACE* anarace, const int windowNumber):UI_Window(parent, STATISTICS_WINDOW_TITLE_STRING, STATISTICS_WINDOW, windowNumber, NOT_SCROLLED)
{
...
void StatisticsWindow::showGraph(DC* dc, int* data, int min, int max, Color col)
{
	int j,k;
...
void StatisticsWindow::draw(DC* dc)
{
	UI_Window::draw(dc);
	for(int i=0;i<20;i++)
		for(int j=0;j<i;j++)
...
//      dc->DrawText(_T(String::Format(T("Tertiary fitness : %i"),tFitness[199])),Legend2.x,Legend2.y+12);
	...
	for(int k=0;k<20;k++)
		...
		{
//            dc->SetTextForeground(*theme.lookUpColor(WINDOW_TEXT_COLOUR));
			dc->SetTextForeground(Color(dc->GetSurface(), 40,100-oldMinsCounter[k]*3,200-oldMinsCounter[k]*8));
			string bla=theme.lookUpFormattedString(MINERALS_STRING, harvestedMins[199]/100);
			int dx,dy;
			dc->GetTextExtent(bla,&dx,&dy);
            ...
            if(oldMins[k]>0) os << "+";
			os << oldMins[k]/100;
			dc->DrawText(os.str(),Legend2.x+dx+oldMinsCounter[k],Legend2.y+24);
		}
