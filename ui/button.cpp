...
{
	if((pressdepth == depth)||(statusFlags & BF_IS_CHECKBUTTON)||(statusFlags & BF_NOT_CLICKABLE))
		return;
...
void UI_Button::process()
{
	...
	if(theme.lookUpButtonColors(buttonColorsType)->type==NO_ANIMATION) {
		gradient = 100;
	} else
		if(!(statusFlags & BF_HIGHLIGHTED)) {
			...
			{
				if((theme.lookUpButtonColors(buttonColorsType)->startBrush[0] == TRANSPARENT_BRUSH) && (text != NULL)) {
					setNeedRedrawMoved();
				} else {
